...
export function FileUploadModal({open,onClose,collectionId,collectionName,onUpload,onIngestPaste,theme}:FileUploadModalProps){
  ...
  useEffect(() => {
    if (open && !uploading && !pasting) {
      // Only reset on clean open (no active processing)
      if (files.length === 0) {
  ...
  return(
    <Modal open={open} onClose={onClose} title="Add Document" wide theme={theme}>
      ...
      {/* Drop zone — always occupies space to prevent layout shift */}
      ...
      {files.length>0&&(
        <div style={{minHeight:300,height:300,overflowY:"auto",marginBottom:8}}>
          {files.map(f=>(
