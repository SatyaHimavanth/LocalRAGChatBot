...
import { Message, Chat, Collection, DocRecord, SearchResult, ToastMsg, Theme, themeVars, getErrMsg, IncompleteJob, AgentPlan, AgentResult, EvidenceEffort } from "./types";
...
useEffect(()=>{
  ...
  const offEvidence = Events.On("chat:evidence", (e:any) => {
    ...
    setStatusMsgs([{ id: crypto.randomUUID(), sender: "system", text: summarizeEvidence(e.data) }]);
  });
  ...
  const submitPrompt = async (
    ...
    const promptForBackend = encodeTurnPrompt(options?.parentMessageId, evidenceEffort, msg);
    setChats((p) =>
      p.map((c) =>
        c.id === tid
          ? {
