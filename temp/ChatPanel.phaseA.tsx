...
import { Message, ThemeVars, Theme, SourceRef, EvidenceEffort } from "../types";
import { I } from "./Icons";
...
export function ChatPanel({
  ...
  const renderFlags = (m: Message, msgSources?: SourceRef[], msgId?: number) => {
    ...
    if (meta?.evidenceEffort) {
      ...
      badges.push(
        <span key="effort" style={flagStyle("rgba(244,114,182,0.12)", "rgba(244,114,182,0.95)", T.border)}>
          Effort {effortLabel}
        </span>
      );
    }
    if (typeof meta?.evidenceCoverage === "number" && meta.evidenceCoverage > 0) {
      badges.push(
        <span key="coverage" style={flagStyle("rgba(245,158,11,0.12)", "rgba(245,158,11,0.95)", T.border)}>
          Coverage {Math.round(meta.evidenceCoverage * 100)}%
        </span>
      );
    }
    if ((meta?.evidencePasses || 0) > 0) {
      badges.push(
        <span key="passes" style={flagStyle("rgba(59,130,246,0.12)", "rgba(59,130,246,0.95)", T.border)}>
          Passes {meta?.evidencePasses}
        </span>
      );
    }
    if ((meta?.sourceCount || 0) > 0) {
      badges.push(
        <button
          ...
          onClick={() => {
            if (hasSources && msgId) {
              setSourceModal({ sources: msgSources!, refNum: msgSources?.[0]?.refNumber || 1 });
            }
          }}
          style={{
            ...flagStyle("rgba(148,163,184,0.12)", T.text2, T.border),
            cursor: hasSources ? "pointer" : "default",
          }}
          title={hasSources ? "Open evidence sources" : "Sources unavailable"}
        >
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", minWidth: 0 }}>
            ...
            {activeChat?.messages.map((m, idx) => {
              ...
              return (
                <div key={m.id} ref={isLastUser ? lastUserMsgRef : undefined} data-msg-id={numericMsgId} style={{ marginBottom: 16, display: "flex", flexDirection: "column", alignItems: m.sender === "user" ? "flex-end" : "flex-start" }}>
                  ...
                  {m.sender === "ai" && (
                    <>
                      {renderFlags(m, msgSources, numericMsgId)}
                      {canCopy && (
                        <div style={{ marginTop: 6, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      <div style={{ padding: "12px 16px", borderTop: "1px solid " + T.border }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, border: "1px solid " + T.border, background: T.inputBg, color: T.text2, fontSize: 11, flexShrink: 0 }}>
            <span style={{ color: T.text3, whiteSpace: "nowrap" }}>Effort</span>
            <select
              value={evidenceEffort}
              onChange={(e) => onEvidenceEffortChange(e.target.value as EvidenceEffort)}
              disabled={isArchived || gen}
              style={{ border: "none", outline: "none", background: "transparent", color: T.text, fontSize: 12, cursor: isArchived || gen ? "default" : "pointer" }}
          ...
          <input value={input} onChange={(e) => onInputChange(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && onSend()} placeholder={isArchived ? "Archived..." : "Ask a question..."} style={{ flex: 1, minWidth: 0, padding: "10px 14px", borderRadius: 8, border: "1px solid " + T.border, background: T.inputBg, color: T.text, fontSize: 13, outline: "none", opacity: isArchived ? 0.4 : 1, transition: "background 0.3s" }} disabled={isArchived} />
